...
def merge_frames(frames):
    dropFrames = ['hash','nonce','blockHash','transactionIndex','txreceipt_status', 'gas','gasPrice','isError','cumulativeGasUsed','gasUsed', 'type','traceId','errCode','confirmations','input','value']
...
def get_token_details(transDf):
    ...
    tempDf['transferValue'] = pd.Series([], dtype='float64')
            ...
            try:
                ...
                if 'result' in details:
                    ...
                    if 'tokenPriceUSD' in tokenDetails:
                        ...
                        tempDf.loc[i, 'transferValue'] = tempDf.loc[i, 'delta'] * tempDf.loc[i, 'tokenValue']
                    else:
                        pass
                else:
                    pass
    ...
    if 'contractAddress' in tempDf:
        logger.debug('Dropping contract address from final frame')
        tempDf.drop(columns=['contractAddress'], inplace=True)
